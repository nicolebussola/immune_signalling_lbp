...
def load_mat(stem: str, label: str) -> pd.DataFrame:
    ...
    mat = (
        pd.read_csv(DATA_DIR / f"mat_{stem}_dreamlet.csv")
        .dropna(axis="columns")
    )
...
dc.plot_volcano_df(
    ...
    x="logFC", y="adj.P.Val",
    top=0, figsize=(10, 10),
    color_pos="#F8083D", color_neg="#045FC4", color_null="#EFECF4",
    lFCs_thr=1,
)
ax = plt.gca()
...
for gene in highlight_genes:
    ...
    ax.scatter(xd, yd, s=60, facecolor="none", edgecolor="black", linewidth=2.0, zorder=10)
    ax.annotate(
        gene, xy=(xd, yd), xytext=(xd + dx, yd + dy),
...
plt.title("CD14+ Monocytes", fontsize=30)
plt.xlabel("logFCs", fontsize=18)
plt.ylabel("-log10(pvals)", fontsize=18)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "CD14_volcano.pdf")
plt.close()
...
sns.heatmap(
    tf_matrix.T, ax=ax,                           
    cmap="icefire", center=0, linewidths=0.5,
    annot=pval_annotations(pval_matrix.T), fmt="",
...
sns.heatmap(
    pathway_acts, ax=ax,
    cmap="coolwarm", center=0, linewidths=4,
    annot=pval_annotations(pval_df), annot_kws={"fontsize": 15}, fmt="",
    cbar_kws={"orientation": "vertical", "shrink": 1},
)
