...
# Selected factors for microglia-monocyte communication.
CELLS_FACTOR_DICT = {
    "micro_blood_coarse": {
...
# Selected factors for neuron-focused communication (Cohort 2 only).
NEURO_FACTOR_DICT = {
    "brain_coarse": {
...
def _enrich_and_plot(
    ...
    enr_lig = enr_df(
        list(filtered_df["ligand"].unique()), genedbs, background_lig, str(enrichr_dir)
    )
    enr_rec = enr_df(
        list(filtered_df["receptor"].unique()),
        genedbs,
        background_rec,
        str(enrichr_dir),
    )
...
def _process_de_strict(ddf, de_count, logfc_threshold=0.5, pval_threshold=0.05):
    ...
    condition2 = (ddf["logfoldchanges"].between(-0.05, 0.05)) & (
        ddf["pvals_adj"] >= pval_threshold
    )
    filtered_df = (
...
def enrich_mono_micro_brain(
    ...
    micro_receptors = filter_and_process_de(
        ranked_genes_df(
            adata_br, lr_mono_to_micro["receptor"].unique(), key="Microglia"
        ),
    ...
    candidate_ligands = lr_mono_to_micro[
        lr_mono_to_micro["receptor"].isin(micro_receptors)
    ]["ligand"].unique()
    mono_ligands = _filter_mono_genes(adata_br, candidate_ligands, target_cells_mono)

    lr_mono_to_micro_filt = filter_lr(
        lr_mono_to_micro.reset_index(), mono_ligands, micro_receptors
    )
    lr_mono_to_micro_filt.to_csv(loadings_path / "lr_loads_mono_to_micro_filtered.csv")
    _enrich_and_plot(
        lr_mono_to_micro_filt,
        background_mono,
        background_micro,
        "Monocytes -> Microglia",
        out_folder,
    ...
    candidate_receptors = lr_micro_to_mono[
        lr_micro_to_mono["ligand"].isin(micro_ligands)
    ]["receptor"].unique()
    mono_receptors = _filter_mono_genes(
        adata_br, candidate_receptors, target_cells_mono
    )

    lr_micro_to_mono_filt = filter_lr(
        lr_micro_to_mono.reset_index(), micro_ligands, mono_receptors
    )
    lr_micro_to_mono_filt.to_csv(loadings_path / "lr_loads_micro_to_mono_filtered.csv")
    _enrich_and_plot(
        lr_micro_to_mono_filt,
        background_micro,
        background_mono,
        "Microglia -> Monocytes",
        out_folder,
...
def enrich_mono_micro_blood_micro(
    ...
    micro_receptors = process_de_genes_celltype(
        adata_br,
        get_unique_receptors(lr_mono_to_micro, mono_ligands),
        n_celltypes_brain,
        celltype="Microglia",
    )
    ...
    micro_ligands = process_de_genes_celltype(
        adata_br,
        lr_micro_to_mono["ligand"].unique().tolist(),
        n_celltypes_brain,
        celltype="Microglia",
    )
    # receptors: must be DE in monocytes in BOTH blood adata and combined adata
...
def enrich_mono_micro_blood_brain(
    ...
    all_ligands = lr_mono_to_micro["ligand"].unique().tolist()
    mono_ligands_bl = set(
        _filter_mono_genes(adata_bl, all_ligands, target_cells_mono_bl)
    )
    ...
    non_mono_brain_receivers = [
        r for r in mo2mi_top_receivers if "Monocytes" not in r and "brain" in r
    ]
    if non_mono_brain_receivers:
        target_cells_tf = [r.split("_")[0] for r in non_mono_brain_receivers]
        mono_ligands = set(
            _filter_mono_genes(adata_tf, list(mono_ligands_bl), target_cells_tf)
        )
    else:
    ...
    all_receptors = get_unique_receptors(lr_micro_to_mono, micro_ligands)
    mono_receptors_bl = set(
        _filter_mono_genes(adata_bl, all_receptors, target_cells_mono_bl)
    )
    ...
    non_mono_brain_receivers = [
        r for r in mi2mo_top_receivers if "Monocytes" not in r and "brain" in r
    ]
    if non_mono_brain_receivers:
        target_cells_tf = [r.split("_")[0] for r in non_mono_brain_receivers]
        mono_receptors = set(
            _filter_mono_genes(adata_tf, list(mono_receptors_bl), target_cells_tf)
        )
    else:
...
def enrich_neuro_micro(
    ...
    """
    LR filtering and enrichment for Neuron↔Microglia.
    ...
    neuro_ligands = _filter_neuro_genes(
        adata_br, lr_n2mi["ligand"].unique(), target_cells=["OPC"]
    )
    # microglia receptors: DE vs. other glial cell types
    candidate_receptors = lr_n2mi[lr_n2mi["ligand"].isin(neuro_ligands)][
        "receptor"
    ].unique()
    micro_receptors = filter_and_process_de(
    ...
    candidate_receptors = lr_mi2n[lr_mi2n["ligand"].isin(micro_ligands)][
        "receptor"
    ].unique()
    neuro_receptors = _filter_neuro_genes(
        adata_br, candidate_receptors, target_cells_neuro_all
    )
...
def enrich_neuro_mono(
    ...
    adata_bl.obs["cell_type_coarse"] = (
        adata_bl.obs["cell_type.v2"]
        .astype(str)
        .apply(lambda x: "Monocytes" if "mono" in x.lower() else x)
    )
    for ref, key in [
        ("GABAergic Neuron", "GABA"),
        ("Glutamatergic Neuron", "Gluta"),
...
def run_enrichment(
    ...
    if cohort in CELLS_FACTOR_DICT.get(factorization_type, {}):
        ...
        print(
            f"No CELLS_FACTOR_DICT entry for {cohort} + {factorization_type} — skipping micro-mono enrichment."
        )

    # Step 2b: Neuro analyses (Cohort 2 only)
    if cohort == "cohort_2":
        if (
            factorization_type == "brain_coarse"
            and "cohort_2" in NEURO_FACTOR_DICT.get("brain_coarse", {})
        ):
            ...
            factorization_type == "brain_blood_coarse"
            and "cohort_2" in NEURO_FACTOR_DICT.get("brain_blood_coarse", {})
        ):
